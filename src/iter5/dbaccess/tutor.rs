...
pub async fn get_all_tutors_db(pool: &PgPool) -> Result<Vec<Tutor>, EzyTutorError> {
    ...
    let tutors: Vec<Tutor> = tutor_rows.iter()
    .map(|tutor_row| Tutor{
...
pub async fn get_tutor_details_db(pool: &PgPool,
    tutor_id: i32) -> Result<Tutor, EzyTutorError> {
        //Prepare SQL statement
        let tutor_row = sqlx::query!("SELECT tutor_id, tutor_name, tutor_pic_url, tutor_profile
        FROM ezy_tutor_c6 WHERE tutor_id = $1", tutor_id)
        .fetch_one(pool)
        .await
        .map(|tutor_row| Tutor{
            tutor_id: tutor_row.tutor_id,
    ...
    -> Result<Tutor, EzyTutorError> {
        let tutor_row = sqlx::query!("INSERT INTO ezy_tutor_c6 (tutor_name, tutor_pic_url, tutor_profile) 
    ...
    pub async fn update_tutor_details_db(pool: &PgPool, tutor_id: i32, update_tutor: UpdateTutor)
    -> Result<Tutor, EzyTutorError> {
        //Retrieve current tutor_row
        let current_tutor_row = sqlx::query_as!(Tutor, "SELECT * FROM ezy_tutor_c6 WHERE tutor_id = $1", tutor_id)
        .fetch_one(pool)
        .await
        .map_err(|err| EzyTutorError::NotFound("tutor id not found".into()))?;
    ...
    -> Result<String, EzyTutorError> {
        ...
        let tutor_row = sqlx::query!(
            "DELETE FROM ezy_tutor_c6 WHERE tutor_id = $1", tutor_id)
            .execute(pool)
            .await?;
